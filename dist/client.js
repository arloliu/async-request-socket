...
class Client {
  ...
  sendRequest(data, options = {}) {
    ...
    if (typeof this._options.attachRequestId !== 'function' || typeof this._options.extractRequestId !== 'function') {
  ...
  _initialConnectingPromise() {
    ...
    return new PromiseTimeout(timeout, `Can't open connection within allowed timeout: ${timeout} ms.`);
  }

  _initialClosingPromise() {
    const timeout = this._options.timeout;
    return new PromiseTimeout(timeout, `Can't close connection within allowed timeout: ${timeout} ms.`);
  }
